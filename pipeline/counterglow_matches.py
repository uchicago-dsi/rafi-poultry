...
def potential_farms(
    infogroup_df: pd.DataFrame,
    counterglow_df: pd.DataFrame,
    states: list,
    max_dist_km: float,
    print_bool=True,
):
...
def infogroup_counterglow_dict(
    sic_df: pd.DataFrame, counterglow_df: pd.DataFrame, states: list, radium_km: float
):
    """
